...
export function exportMap(map, options = exportOptions) {
  return new Promise((resolve) => {
    ...
    const filename = options?.title?.toLowerCase()?.split(' ').join('-') || 'map';
    ...
    const northArrowHeightOffset = calculateFrom150(titleTextHeightOffset + 10, dpi);
    ...
    map.once('rendercomplete', () => {
      ...
      async function prepareMap() {
        ...
        if (options.title) {
          ...
          const { actualBoundingBoxAscent, actualBoundingBoxDescent } = textDims;

          // title background
          mapContext.fillStyle = 'rgba(255,255,255,0.85)';
          mapContext.fillRect(
            transformedWidth * 0.5 - titleWidth * 0.5 - titlePadding,
            titleTextHeightOffset + titleFontSize - actualBoundingBoxAscent - titlePadding * 0.8,
        ...
        if (options.legend) {
          ...
          mapContext.drawImage(legendCanvas, legendOffset, transformedHeight - legendCanvas.height - legendOffset);
        }
